...
void EnemyPursuit(int enemyNum) {
	...
	for (int i = 0; i < ENEMYNUMBER; i++) {
		...
		case enemyKind01:
			if (g_enemy[i].bulletFreamCount < g_enemy[i].firingInterval - 5) {//弾発射フレームより-5フレーム未満だったら中に入る
			...
			if (g_enemy[i].bulletFreamCount >= g_enemy[i].firingInterval - 5) {//発射フレームの-5フレーム以上あれば中に入る
